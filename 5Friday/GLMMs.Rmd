---
title: "Beyond GLMs with random effects"
...
- Beyond GLMs: mixed-effects models
- Practical 13: GLMMs
...
\begin{itemize}
\item \textbf{Bolker et al. (2009)} Generalized linear mixed models: a practical guide for ecology and evolution. \textit{Trends in Ecology \& Evolution}
\item \href{https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html}{\textbf{Ben Bolker's GLMM FAQ}}: common issues, convergence, random effects
\item \textbf{Zuur et al. (2009)} Mixed Effects Models and Extensions in Ecology with R. Springer
\item \href{https://www.physalia-courses.org}{\textbf{Florian Hartig's Physalia course}}: GLMMs in R
\end{itemize}
...
- An effect is "unobserved" or "latent"
- We do not want our model to "depend" on the effect
- To account for pseudoreplication
- Something that is nuisance (e.g., library depth when not measured)
- To incorporate correlation between observations (space, time, individuals, genes, ...)
- To estimate variation of an effect
- To incorporate overdispersion
- To estimate fixed effects at the "population level"

There is more.

# "Technicalities"

## Why is it called a random effect?

"Random" is in contrast to "fixed"

- Fixed effects we maximize
- Random effects we marginalize

Random effects are *random* or *stochastic*; like our data they come from a distribution

\pause

\textcolor{red}{Mixed effects means a model that incorporates both fixed and random effects}
...
- But now, random effects are from a distribution
- This allows us to incorporate correlation

## Our new likelihood

\begin{equation}
\mathcal{L}(\textbf{y};\symbf{\Sigma}, \Theta) = \displaystyle \int \prod \limits^n_i f(y_i ;\Theta\vert \textbf{u}) f(\textbf{u};\symbf{\Sigma}) d\textbf{u}
\end{equation}
...
For non-normal responses (GLMMs), the marginal likelihood usually has no closed form and the integral must be approximated.
...
We assume that the random effects follow a normal distribution:

$$
\textbf{u} \sim \mathcal{N}(\textbf{0}, \boldsymbol{\Sigma})
$$
However, we are really interested in $\textbf{u}\vert\textbf{y}$ instead; this holds information from our data!

- We want $f(\textbf{u}\vert\textbf{y})$ to be as flexible as possible
- So we can capture the patterns in the data as accurately as possible
...
\textcolor{red}{A few examples next}
...
plot(ggeffects::predict_response(model1, terms = "Nest", type = "random", interval = "confidence"))+see::scale_color_flat()+ggplot2::theme_classic(base_size=20)+ ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, vjust = 1, hjust=1))
```
...
## Example: random slopes, intercepts, and their correlation in \texttt{glmmTMB}
...
ggplot(data.frame(x=0), aes(x=x))+stat_function(fun = dnorm, n = 101, args = list(mean = 0, sd = 1))+theme_classic()+geom_vline(aes(xintercept=0), col="red",lty="dashed")+theme(text = element_text(size=24))+xlim(c(-3,3))+xlab("Random effect")+ylab("Likelihood")+annotate("text",x=0.6,y=0.35,label="mean = mode",col="red",size=6)

# gamma: shape=2, scale=2 -> mode=(k-1)*scale=2, mean=k*scale=4
ggplot(data.frame(x=0), aes(x=x))+stat_function(fun = dgamma, n = 101, args = list(shape = 2, scale = 2))+theme_classic()+geom_vline(aes(xintercept=2), col="red",lty="dashed")+geom_vline(aes(xintercept=4), col="blue",lty="dashed")+theme(text = element_text(size=24))+xlim(c(0,14))+xlab("Random effect")+ylab("Likelihood")+annotate("text",x=2,y=0.17,label="mode",col="red",size=6,hjust=-0.1)+annotate("text",x=4,y=0.17,label="mean",col="blue",size=6,hjust=-0.1)
...
\begin{center}
\textcolor{red}{These are important concepts to understand error messages and convergence in mixed effects models.}
\end{center}
...
- Be even more suspicious of information criteria
- Different packages have different benefits (\texttt{glmmTMB} vs. \texttt{lme4})
- Do not blindly assume approximations perform well
- Always check (residual) assumptions
- When in doubt: simulate from your fitted model (e.g., \texttt{DHARMa})