...
plot(y ~ x, ylim = range(c(y, predictions3$fit)), type = "n")
...
1. Make a plot of your data and the prediction. For a multiple linear regression, you may want to try for various predictor variables $\textbf{x}$. Does the regression line capture the trend in the data well?

2. Add a confidence interval band to the prediction. Where is the uncertainty largest, and why?

3. If you have two continuous covariates in the model, fix one at its mean and plot the effect of the other. How does the choice of fixed value change the plot?

4. Construct a caterpillar plot of the coefficients. Which predictors have confidence intervals that do not cross zero?

5. Use `effects::allEffects` or `ggeffects::ggeffect` to visualize your regression. How does this compare to the plot you made manually?

# Part 2

<details><summary>Open after Part 1</summary>

Try reconstructing your plots from Part 1 using the `ggplot2` package.

1. Start with `ggplot(data = ..., aes(x = ..., y = ...))` and add `geom_point()` to plot the observations.
2. Add the regression line using `geom_line()`, passing the predictions as a separate data frame via the `data` argument.
3. Add a confidence interval band with `geom_ribbon(aes(ymin = LI, ymax = UI))`.
4. Reconstruct the caterpillar plot using `geom_point()` and `geom_errorbarh()` on the coefficients and confidence intervals.

</details>

[If you finish early you can pick a different dataset, or there is also the 2024 edition of this practical.](https://htmlpreview.github.io/?https://github.com/BertvanderVeen/GLM-workshop/blob/main/Practicals/2024/5Practical.html) \newline
