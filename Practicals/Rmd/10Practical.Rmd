...
Because Poisson regression uses the log link, the coefficients are on the log scale. A coefficient of $\beta$ means that a one-unit increase in a predictor multiplies the expected count by $e^\beta$. Because the exponential function is monotonic, we can simply exponentiate the endpoints of a confidence interval to get a valid interval on the count scale:
...
```{r, fig.width = 10, seed = 2}
(y2 <- rnbinom(n, mu = lambda, size = 1))
hist(y2)
```
...
```{r, fig.width = 10}
plot(log(fitted(reg2)), residuals(reg2, type = "pearson"),
     xlab = "log(Fitted values)", ylab = "Pearson residuals")
abline(h = 0, col = "red")
```

we see that they look poor. Typically, when there is overdispersion present in the model, residuals will "fan out", i.e., the spread increases with the fitted values (horizontal axis). This pattern is typical when there is an issue of overdispersion. Instead of looking at the residuals, and identifying overdispersion visually, we can also quantify the overdispersion with an overdispersion factor:
...
1. [Horseshoecrabs](https://raw.githubusercontent.com/BertvanderVeen/GLM-workshop/refs/heads/main/data/Horseshoecrabs.csv): Data on the number of male satellites attracted by female horseshoecrabs, data used in the presentation today.
...
2. Examine the parameter estimates. Use `exp(coef(...))` to interpret them on the count scale. What does a one-unit change in a predictor mean for the expected count?

3. Examine the confidence intervals with `confint()`. You can also exponentiate them with `exp(confint(...))`. What conclusions can you draw about the effects?

4. Check the residuals, and test with the `DHARMa` or `performance` packages if there is overdispersion.

# Part 2

<details><summary>Open after Part 1</summary>

5. Fit a negative-binomial regression and check the residuals for improvement.

6. Compare the parameter estimates from both models. Have they changed?

7. Compare the confidence intervals. Have they gotten wider or narrower? What does this tell you about what overdispersion does to uncertainty in the Poisson model?

8. Has your overall conclusion about the effects changed?
