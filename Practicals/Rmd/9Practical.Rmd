...
The only thing we need to do is work out what $\textbf{C}$ should look like. The rows of $\textbf{C}$ represent the new parameter estimate that we want, as a function of the parameters that we already have (the columns), here that is $\beta_1$ and $\beta_3$. For the parameters that we need to sum, we place a 1 in the correct row and column. For the example above, where there are three groups and group 2 is the reference category, if we want the effect for group 1, $\textbf{C}$ is:
...
`marginaleffects` makes it a bit easier for us to get the results on the response scale, by switching to `type = "response"`, similar as in the `predict` function. It also includes separate functions for plotting the comparisons; `plot_comparisons` and `plot_predictions`.
...
1. Fit a model to one of the datasets that includes a categorical predictor variable
