...
In the first lecture, $p$ was the same for all observations. Now, as in a GLM, $p$ is observation-specific. Because $p$ was different per observation, a-priori guessing the number of 1s and 0s in the data is much more challenging. Because the `size` argument was set to 1, we only simulate 1s and 0s. 
...
2. Study the parameter estimates and their uncertainty. The coefficients are on the log-odds scale. What can you conclude about the direction of the effects?

3. Exponentiate the coefficients and confidence intervals with `exp(coef(...))` and `exp(confint(...))` to get odds ratios. Because the exponential function is monotonic, exponentiating the endpoints gives a valid interval on the odds scale. How do you interpret an odds ratio greater than 1? Less than 1?

4. Use `predict(model, type = "response", newdata = ...)` to get predicted probabilities for two specific sets of conditions that differ in one predictor. How does the probability change?

5. Use `marginaleffects::plot_slopes(model, variables = "x", condition = "z")` to visualise how the effect of one predictor on the probability scale varies across values of another predictor. Then use `marginaleffects::avg_slopes(model)` to summarise the average marginal effect across all observations. How does this compare to simply reading off the regression coefficient?

# Part 2

<details><summary>Open after Part 1</summary>

6. Compare the change in probability from Q4 to the corresponding odds ratio from Q3. An odds ratio of 2 does not mean the probability doubles — why not? How does the effect of a predictor on the probability scale depend on the baseline probability?

7. How do the parameter estimates change when you switch from a logit to a probit link?

8. Plot the residuals of the model. How do they look? Can you discern any assumption violations?
