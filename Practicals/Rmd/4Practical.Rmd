...
The `anscombe` dataset is built into R and can be loaded with `data(anscombe)`. It contains four pairs of x and y variables (`x1`/`y1` through `x4`/`y4`) that all produce nearly identical regression summaries, but look very different.

1. Fit a simple linear regression for each of the four datasets: `lm(y1~x1)`, `lm(y2~x2)`, etc. Compare the coefficients and $R^2$ across models.
2. Use `plot` to visualize the residuals of each model.
3. Which assumptions are violated in each case, and how can you tell?
...
- normality: can be pragmatically addressed by normalizing the data (`log(y)` for example) before regression
...
- homoscedasticity: can be pragmatically addressed by applying a variance-stabilizing transformation (`sqrt(y)`for example) before regression, or by incorporating a missing covariate
...
[If you finish early you can pick a different dataset, or there is also the 2024 edition of this practical.](https://htmlpreview.github.io/?https://github.com/BertvanderVeen/GLM-workshop/blob/main/Practicals/2024/4Practical.html) \newline
