...
1. To begin, can you briefly summarize (in your own words) what the difference is between confirmatory and exploratory analysis?

2. Can you calculate deviance by hand for the binomial distribution? *Hint: it is included in today's lecture slides*

3. Formulate a hypothesis, fit two models, and use `anova` to compare them. Do you accept or reject the alternative hypothesis?

4. Use `anova` with the most complex of the two models in 3 (or an even more complex model) to get an analysis of deviance table. Then apply `car::Anova(model, type = "II")` and `car::Anova(model, type = "III")` to the same model. Do all three give the same result?

<details><summary>Why don't they give the same answer?</summary>

The three types differ in *what each term is tested against*:

- **Type I (sequential):** terms are added one at a time in the order you wrote the formula. Each term is tested against the model without it, but *including* all terms that came before it. The result therefore depends on the order of terms in the formula.
- **Type II (marginal, no interactions):** each main effect is tested against a model that includes all other main effects, but *not* higher-order terms involving that effect. Order does not matter. This is usually the right choice when there are no interactions, or when you want to test main effects in the presence of other main effects only.
- **Type III (marginal, all terms):** each term is tested against a model that includes *all* other terms, including interactions. This means a main effect is tested while its interaction is in the model, which can give misleading results when interactions are present and the predictors are not centred.

Try reordering the predictors in your formula and applying `anova` again. Notice that the Type I results change, while Type II and III do not.
...
5. Compare your confirmatory results to the model comparison with AIC/AICc/BIC from Practical 7 (the exploratory analysis). Do the two approaches agree on which model is "best"? Can you think of reasons why they might or might not overlap?
