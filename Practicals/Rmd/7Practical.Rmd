...
To calculate AIC and BIC, we first need a model. Let's take the simulation from the previous practical:
...
we received a warning, indicating that the model is overfitting, but for the moment we will ignore that. AIC and BIC are used for comparison between models by examining their difference, which is generally denoted $\Delta AIC$ and $\Delta BIC$. There exist rules of thumb for when we should consider one model better than another; generally the rule maintained is that if the alternative model's AIC or BIC is at least 2 points lower than that of the first model, per added parameter, we can consider it to be better. If the difference is less than that, we prefer the simpler model per the principle of parsimony. Here, we added one parameter (for the interaction of the two predictors), so we need the AIC or BIC to be 2 points lower than for the first model:
...
Such tables should be used cautiously for model comparison. Years ago, groups of researchers got so caught up in model comparison that studies would report entire tables of all potential submodels of the most complex model that was possible to fit for a given dataset. Such tables (and the submodels) can automatically be fitted using an approach called "dredging", which is the automatic fitting of models, and ranking following some information criteria. As a demonstration, we can here use the `dredge` function from the `MuMIn` package. We apply this to the second model, as it is more complex than the first, and the first is nested in the more complex model:
...
Note: `dredge` requires the model to have `na.action = na.fail`, otherwise it will throw an error. You can set this per model as above, or globally at the start of your script with `options(na.action = na.fail)`.

To prevent `dredge` from fitting models where an interaction is included but its constituent main effects are not, use the `subset` argument with `dc()` (dependency constraint). `dc(x, ...)` means: if term `x` is in the model, all `...` must also be present:

```{r, eval = FALSE}
MuMIn::dredge(model, subset = dc("A:B", "A", "B"))
```

For multiple interactions, chain constraints with `&`:

```{r, eval = FALSE}
MuMIn::dredge(model, subset = dc("A:B", "A", "B") & dc("A:C", "A", "C"))
```
...
which returns our original (true) model. There is an issue that can arise here: models can automatically be fitted where the interaction term is included, but the main model terms are not. Such models are almost never sensible, and the general recommendation is to only consider models that include lower-order terms (i.e., main effects) when higher-order terms are present. The `dredge` function has all kinds of possibilities for customising, you can also rank models with BIC, $R^2$ or otherwise, but we do not explore this further here.
...
4. [OME]: Counts of correct identifications, used in the presentation ([MASS package](https://rdrr.io/cran/MASS/man/OME.html)). Load with `data(OME, package = "MASS")`
...
4. Before moving on: write down the model you landed on as "best" and your full ranking of the models you compared. Are you confident you considered every plausible submodel of your full model?

5. Now use `MuMIn::dredge` to perform an exhaustive search. Did it find the same best model as you did in Q3? If the rankings differ, can you identify which models you missed or ordered differently?

6. When would dredging be a bad idea? Think about: the number of predictors relative to sample size, the difference between exploratory and confirmatory analysis, and what happens to p-values and confidence intervals of the "winning" model after selection.

# Part 2

<details><summary>Open after Part 1</summary>

7. Repeat steps 1-3 with `AICc` and/or with `BIC`. Are there differences in the comparisons when you use a different information criterion?

8. Compare the confidence intervals of the parameter estimates that are present in both your original model and in the final model from one of the selection procedures. How have they changed?
