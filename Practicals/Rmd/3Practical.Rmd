...
Yesterday, we learned about sampling variation, uncertainty, t-tests and simple linear regression. In this practical we will focus on linear regression, for which we can simulate some data:
...
In this simulation, $\textbf{x}$ is a predictor variable simulated between 3 and 5 (so it takes only positive values between those numbers), and $\textbf{y} = \alpha + \textbf{x}\beta + \boldsymbol{\epsilon}$ is a response variable with intercept $\alpha = 1$ (where $x = 0$), slope for the predictor $\beta = 2$, and error term $\boldsymbol{\epsilon}$ with mean zero and standard deviation one.
...
5. normtemp (load with `data(normtemp, package = "UsingR")`): Body temperature and resting heart rate of 130 individuals ([Mackowiak et al. 1992](https://doi.org/10.1001/jama.1992.03490120092034))

*Note: this is a large practical and you may not finish all parts. You can continue later if time allows.*
...
2. Identify potential predictor variables $\textbf{x}$ for a multiple linear regression.
...
6. Overall, what conclusions can you draw for the relationship of $\textbf{y}$ and $\textbf{x}$?
7. Simulate a random variable `z <- rnorm(nrow(dataset))` and add it to your multiple regression. Is it significant? How does its inclusion affect the estimates and confidence intervals for the real predictors?

</details>

# Part 3: interactions

<details><summary>Open after Part 2</summary>

Load the `normtemp` dataset (and convert the temperature to Celsius with `(temperature - 32) * 5/9` if preferred). Convert `gender` to a factor with `factor(gender, labels = c("Male", "Female"))`.

1. Fit a multiple regression with `gender` and `hr` as predictors. What is the interpretation of the intercept and each coefficient?
2. Change the reference category using `relevel`. How does the model summary change, and what does it now mean?
3. Fit a model that includes the interaction between `gender` and `hr`. Does the effect of heart rate on body temperature differ between males and females?
4. Plot the fitted regression lines for each group.
5. A quadratic term is a special type of interaction — the variable interacts with itself. Add a quadratic term for `hr` with `I(hr^2)`. Does body temperature have a nonlinear relationship with heart rate? How does this change your interpretation?

</details>

[If you finish early you can pick a different dataset, or there is also the 2024 edition of this practical.](https://htmlpreview.github.io/?https://github.com/BertvanderVeen/GLM-workshop/blob/main/Practicals/2024/3Practical.html)
