...
```{r}
model1 <- lm(Sources ~ SampleDepth+Season, data = ISIT)
coef(model1)
model2 <- lm(Sources ~ SampleDepth*Season, data = ISIT)
model2 <- lm(Sources ~ SampleDepth+Season + SampleDepth:Season, data = ISIT)
coef(model2)
```
...
```{r}
model3 <- lm(Sources ~ SampleDepth, data = ISIT)
coef(model3);coef(model1)[1:2]
model4 <- lm(Sources ~ Season, data = ISIT)
coef(model4);coef(model1)[c(1,3)]
```

4. How does the change in the coefficients affect the answer to your question?

```{r}
# intercept is lower for first, higher for second
# effect of sample depth is similar
# mean of season is different lower
```
...
```{r}
confint(model1) 
confint(model3) # CI sample depth is similar
confint(model4) # CI season is a bit less wide for model 1 
```

6. Would centering one (or multiple) covariates in your model improve its interpretability?

```{r}
coef(lm(Sources ~ I(SampleDepth-mean(SampleDepth))+Season, data = ISIT))[1]
coef(model3)[1]
# bioluminescense at mean depth instead of at the surface
```
...
```{r}
model5 <- lm(Sources ~ SampleDepth+I(SampleDepth^2)+Season, data = ISIT)
model5
```
...
```{r}
y <- ISIT$Sources+.1
MASS::boxcox(lm(y ~ SampleDepth, data = ISIT))
```
...
```{r}
lm(Sources ~ Season, data = ISIT)

ISIT2 <- ISIT
ISIT2$Season <- relevel(ISIT2$Season, ref=2)
lm(Sources ~ Season, data = ISIT2)
```
...
```{r}
contrasts(ISIT2$Season) = contr.sum(2)
lm(Sources ~ Season, data = ISIT2)
```
