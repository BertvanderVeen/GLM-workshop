...
```{r}
hist(y<-rnorm(10)) # does not really look like a normal distribution
```
...
```{r}
hist(y<-rnorm(1000)) # looks like a normal distribution
mean(y) # much closer than the 0 mean we simulated from
var(y);sd(y) # close to 1 that we simulated from
```
...
```{r}
sim1 <- replicate(100, rnorm(10, mean = 2))
sim2 <- replicate(100, rnorm(10, mean = 6))
apply(sim1, 2, mean)
apply(sim2, 2, mean)
```
...
```{r}
quantile(apply(sim1, 2, mean), probs = c(0.025, 0.975))
quantile(apply(sim2, 2, mean), probs = c(0.025, 0.975))
```

8. Can you conclude that the two simulations have different means?

```{r}
# Yes, the mean of sim1 is not included in the intervals of sim2
# and vice versa
```
...
```{r}
sim1 <- replicate(100, rnorm(10, mean = 2, sd = 5))
sim2 <- replicate(100, rnorm(10, mean = 6, sd = 5))
means1 <- apply(sim1, 2, mean);mean(means1) # pretty close
means2 <- apply(sim2, 2, mean);mean(means2) # prett close
quantile(apply(sim1, 2, mean), probs = c(0.025, 0.975))
quantile(apply(sim2, 2, mean), probs = c(0.025, 0.975))
# variance of mean estimator has gotten much larger. Interval of sim2 now includes mean of sim1
```
...
```{r}
# using a new simulation for brevity
y <- rnorm(100, mean = 5, sd = 2)
ll <- function(y, mean, sd)sum(dnorm(y, mean, sd, log = TRUE))
means <- seq(from = -10, to = 10, length.out=1000)
plot(means, sapply(means, ll, y = y, sd = 2), type = "l")
```

12. What is the maximum likelihood estimate for the mean of the simulations, calculated by the `optim` function?

```{r}
optimise(ll, y = y, sd = 2, lower = -100 , upper = 100, maximum = TRUE)
```
