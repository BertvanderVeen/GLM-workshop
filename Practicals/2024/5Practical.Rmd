...
At this point in the workshop we have covered 1) Sampling variation and maximum likelihood estimation, 2) Simple linear regression, 3) Multiple linear regression, and 4) Checking model assumptions. With this practical, we will end the second day of the workshop with something fun: making nice plots of our data and model results. 
...
Some useful R-packages for plotting are:
...
In the previous practical we focused on learning about the assumptions of multiple linear regression, and how to check them using plots of residuals. In this practical, we explore how we can present model results visually with different types of plots. After all, plots usually convey much more information in a more intuitive way than tables. Generally, only use tables for presenting model results in the main text of a report of article if the actual numbers matter to the narrative of the article.
...
1. How would you visualize an ANOVA?
...
4. What plots can you use to visualize an ANCOVA?

## base R plotting

5. Plot a multiple regression together with the data. *Hint: plot your data first, and add the model after*

6. Use `par(mfrow=c(..,..))` to plot multiple base R plots together.

7. Use the `predict` function to (also) plot your regression line beyond the range of your data.
