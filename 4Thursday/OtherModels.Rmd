...
\begin{tabular}{lll}
\toprule
\textbf{Data type} & \textbf{Example} & \textbf{Model} \\
\midrule
Positive continuous & Biomass, rainfall & Gamma, log-normal, Tweedie \\
Proportions (known $N$) & Successes/failures & Binomial \\
Proportions (unknown $N$) & Coverage, win rate & Beta regression \\
Ordered categories & Ratings, cover classes & Ordinal regression \\
Nominal categories & Species, vote share & Multinomial regression \\
Counts with excess zeros & Rare species counts & ZIP, ZINB, hurdle \\
\bottomrule
\end{tabular}
\end{center}
...
Interpretation is \emph{very} difficult with inverse-type link functions
...
```{r, echo = FALSE, fig.height=8, fig.width=13, fig.align="center", out.height="0.7\\textheight"}
par(mfrow=c(2,2))
...
hist((y<-rbeta(1000,shape1, shape2)), xlab = expression(y[i]), main = bquote(mu == .(meanb(shape1,shape2)) ~ "," ~ sigma^2 == .(varb(shape1,shape2))), cex.main = 2, cex.lab = 1.5, ylab = "Frequency")
shape1 <- 5;shape2<-1
hist((y<-rbeta(1000,shape1, shape2)), xlab = expression(y[i]), main = bquote(mu == .(meanb(shape1,shape2)) ~ "," ~ sigma^2 == .(varb(shape1,shape2))), cex.main = 2, cex.lab = 1.5, ylab = "Frequency")
shape1 <- 1;shape2<-3
hist((y<-rbeta(1000,shape1, shape2)), xlab = expression(y[i]), main = bquote(mu == .(meanb(shape1,shape2)) ~ "," ~ sigma^2 == .(varb(shape1,shape2))), cex.main = 2, cex.lab = 1.5, ylab = "Frequency")
