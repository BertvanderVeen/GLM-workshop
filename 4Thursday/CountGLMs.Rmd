...
\textbf{The Poisson parameter $\lambda$ is the mean of the counting process}

## Is Poisson regression in the EF?

\begin{equation}
\mathcal{L}(y_i;\Theta) = \text{exp}\biggl\{\frac{y_i\textcolor{blue}{\log(\lambda)}-\textcolor{red}{\lambda}}{1} \textcolor{orange}{- \log(y_i!)}\biggr\}
\end{equation}
...
- Number of pigeons in a city the size of Los Angeles
...
Our assumption: $\lambda = \text{var}(y_i)$ \newline
Reality: $\lambda \leq \text{var}(y_i)$
...
Our assumption: $\lambda = \text{var}(y_i)$ \newline
Reality: $\lambda \geq \text{var}(y_i)$
