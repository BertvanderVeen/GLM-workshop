...
- Independence: Partial residual plots, Residuals vs. fitted, Residuals vs. lagged residuals
...
\begin{equation}
D(\textbf{y};\hat{\symbf{\mu}}) = \sum \limits^n_{i=1} 2\bigl[y_i\{g(y_i)-g(\hat{\mu}_i)\} - y_i + \hat{\mu}_i\bigr]
\end{equation}
...
- $d_i = 2\bigl[y_i\{g(y_i)-g(\hat{\mu}_i)\} - y_i + \hat{\mu}_i\bigr]$
...
\begin{equation}
\hat{\epsilon_{i,deviance}} = \text{sign}(y_i - \hat{\mu}_i)\sqrt{d_i}, \qquad \text{so that } \sum \limits^n_{i=1} \hat{\epsilon_{deviance,i}}^2 = D(\textbf{y};\hat{\symbf{\mu}})
\end{equation}
...
```{r, echo = FALSE, fig.height = 8, fig.width = 13, warning= FALSE, message=FALSE}
par(mfrow=c(1,3))
...
model2 <- glm(Sat ~ SpineBothOK + SpineOneOK + SpineNoneOK + ColourMedium + ColourDark.medium + ColourDark + Width + Weight, family = "poisson", data = hcrabs)
```
