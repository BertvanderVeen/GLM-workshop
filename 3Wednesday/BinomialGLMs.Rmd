...
\item Two key components: the distribution and the link function
  \item They link the systematic and the random components
\end{itemize}
...
\textbf{Goal}: estimate $p_i$ for each observation

![Jacob Bernoulli](Bernoulli.jpg){height=80%}
...
- Occurrence of species: orchids \emoji{upside-down-face}
...
library(tidyr)
knitr::kable(head(OME), format="latex", booktabs = T)%>%kableExtra::kable_styling(position = "center")
```
\normalsize
...
par(mfrow=c(1,2))
plot(OME$Correct, x=OME$Noise, ylab = "Correct identifications", xlab="Noise stimulus", ylim = c(0, 13))
plot(OME$Trials-OME$Correct, x=OME$Noise, ylab = "Incorrect identifications", xlab="Noise stimulus", ylim = c(0, 13))
```
...
```{r, echo = FALSE, fig.height = 6}
par(mfrow=c(1,2))
boxplot(Correct~Noise+Age, ylab = "Correct identifications", xlab="Noise stimulus and Age (months)", OME, col = as.numeric(OME$Noise)+2, ylim = c(0,13))
boxplot(Trials-Correct~Noise+Age, ylab = "Incorrect identifications", xlab="Noise stimulus and Age (months)", OME, col = as.numeric(OME$Noise)+2, ylim = c(0,13))
```
...
par(mfrow=c(1,1))
plot(predict(model, newdata = data.frame(Noise = "coherent", Age = c(1:60)), type = "response"), xlim = c(1,60), type = "l", ylim = c(0.5,1), col = 3, ylab = "Probability of correct detection", xlab = "Age")
text(x=55,y=0.75, "Coherent")
lines(predict(model, newdata = data.frame(Noise = "incoherent", Age = c(1:60)), type = "response"), x=1:60, col = 4)
rug(OME$Age, lwd = 2)
text(x=55,y=0.94, "Incoherent")
```
...
<!--Incoherent signals are on average well identified, but coherent signals aren't -->
...
- for binomial distribution: $\textcolor{blue}{\eta_i = \log(\frac{p_i}{1-p_i})}$, $\textcolor{pink}{a(\phi) = 1/N}$, $\textcolor{red}{b(\eta_i) = -\log(1-p_i)}$, $\textcolor{orange}{c(y_i, \phi) = \log\binom{N}{r}}$
- canonical link
...
<!-- \begin{equation} -->
<!-- \mathcal{L}(y_i;\Theta) = \text{exp}\biggl\{\log(1) + \frac{r\textcolor{blue}{\log(\frac{p_i}{1-p_i})}+\textcolor{red}{\log(1-p_i)}}{1}\biggr\} -->
<!-- \end{equation} -->
...
<!-- - for some known functions $\textcolor{pink}{a(\cdot)}$, $\textcolor{red}{b(\cdot)}$ and $\textcolor{orange}{c(\cdot)}$ -->
<!-- - for Bernoulli distribution: $\textcolor{blue}{\eta_i = \log(\frac{p_i}{1-p_i})}$, $\textcolor{pink}{a(\phi) = 1}$, $\textcolor{red}{b(\eta_i) = -\log(1-p_i)}$, $\textcolor{orange}{c(y_i, \phi) = \log(1)} = 0$ -->
<!-- - canonical link -->
...
\begin{itemize}
\item Logit: $\log(\frac{p_i}{1-p_i})$ and inverse $\frac{\exp(\eta_i)}{1+\exp(\eta_i)}$ - \textit{the canonical link}
\item Probit: $\Phi^{-1}(p_i)$ and inverse  $\Phi(\eta_i)$
\item Complementary log-log: $\log(-\log(1-p_i))$ and inverse $1-\exp(-\exp(\eta_i))$
\item \sout{Log-log} \tiny (rarely useful) \normalsize
\end{itemize}
...
\begin{equation}
\begin{aligned}
\text{pr}(y_i = 1) = p_i &= \frac{\exp(\eta_i)}{1+\exp(\eta_i)}\\ \pause
\log(\frac{p_i}{1-p_i}) = \eta_i &= \textbf{x}_i^\top\symbf{\beta}
\end{aligned}
\end{equation}
...
In probit regression, we use a latent variable $\eta^\star_i$ for thresholding. \pause
...
\begin{equation}
\eta^\star_i = \textbf{x}_i^\top\symbf{\beta} + \epsilon_i\sim\mathcal{N}(0,1)
\nonumber
\end{equation}
...
\begin{aligned}
p_i &= \Phi(\eta_i)\\
\eta_i &= \textbf{x}_i^\top\symbf{\beta}
\nonumber
\end{aligned}
\end{equation}
...
plot(y=dnorm(x),x,type="l", ylab = expression(paste("dnorm(", eta[i]^"*", ")")), xlab=expression(eta[i]^"*"))
...
In logistic regression, we can also use a latent variable $\eta^\star_i$ for thresholding. \pause

Probit regression: $\epsilon \sim \mathcal{N}(0,1)$ \newline
Logistic regression: $\epsilon \sim \mathcal{Logistic}(0,1)$
...
eta = -1
x <- seq(-5,5,length.out=1000)
plot(y=dlogis(x),x,type="l", ylab = expression(paste("dlogis(", eta[i]^"*", ")")), xlab=expression(eta[i]^"*"))
axis(1, at = c(-4,-2,-1,0,1,2,4))
polygon(c(x[x<eta],rev(x[x<eta])),c(rep(0,sum(x<eta)),rev(dlogis(x[x<eta]))), col = "red")
```
...
\eta_i &= \textbf{x}_i^\top\symbf{\beta}\\
\eta^\star_i &= \textbf{x}_i^\top\symbf{\beta} + \epsilon_i\sim\mathcal{Logistic}(0,1)
\end{aligned}
\end{equation}
...
- Beta-binomial GLM
- Quasibinomial GLM: inflates standard errors by the overdispersion factor, but does \textbf{not} change point estimates
- Mixed-effects model See [GLMM workshop](https://www.physalia-courses.org/courses-workshops/glmms-in-r/)
...
- Binomial GLMs for binary or count (successes/failures) responses
