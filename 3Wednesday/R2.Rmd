...
- Negative with $p>n$, otherwise always lower than $R^2$
...
\begin{equation}
R^2_k = 1 - \frac{\sum \limits^n_{i=1}  (y_i - \mu_i)^2}{\sum \limits^n_{i=1}  (y_i - \alpha - x_{ik}\beta_k)^2}
\end{equation}
...
\begin{itemize}
\itemsep-0.5em 
\item the numerator represents the unexplained variation of a model
\item the denominator represents the explainable variation of a \textbf{covariate}
\item One minus their fraction is the explained variation
\end{itemize}
...
\begin{itemize}
\itemsep-0.5em 
\item Quantifies the variation additionally explained in a more complex models
\item Is sensitive to order of covariates
\item Multicollinearity: two covariates can explain similar variation
\end{itemize}
...
$R^2$ is linear models is based on the error \newline
...
\begin{equation}
\text{McFadden }R^2 = 1-\frac{\log\mathcal{L}(\textbf{y};\Theta_1)}{\log\mathcal{L}(\textbf{y};\Theta_0)}
\end{equation}
...
```{r, echo = FALSE, fig.height = 3}
hist(R2s, xlab = expression(R[deviance]^2), breaks = 100, cex.main = 1.5, main="Red line = observed R2")
abline(v=devianceR2[2], col = "red", lty="solid", lwd = 3)
```
