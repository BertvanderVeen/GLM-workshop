...
<!-- Replacing y_i by its expectation E[y_i] = n_picks*p in the observed Fisher information
     gives the expected Fisher information I(p) = n_times*n_picks / [p*(1-p)],
     which is where the true p in the CLT variance comes from. -->

In the limit as $n_{times} \to \infty$:

\begin{equation}
\hat{p} \sim \mathcal{N}\!\left(p,\; \frac{p(1-p)}{n_{times}\cdot n_{picks}}\right) \nonumber
\end{equation}

\pause

- Precision grows with more field visits \textit{and} more picks per visit
- Underpins the $\hat{p} \pm 1.96\,\text{SE}$ confidence interval
...
From this we also know $\text{var}(\hat{p}) = \frac{p(1-p)}{n_{times}}$.
