...
n.fields = 1 # number of fields sampled
# Collect data
y <- rbinom(n.fields, size = n.picks, prob = p.orchid) 
y/n.picks # Proportion of orchids
```

## Simulation: law of large numbers

What if we sample many fields with a single pick each?

```{r, cache=TRUE}
set.seed(12345) # For reproducibility
n.fields = 1e5 # The number of fields sampled
n.picks <- 1 # The number of picks in the field
 # Collect data
y <- rbinom(n.fields, size = n.picks, prob = p.orchid)
...
set.seed(12345) # For reproducibility
n.fields <- 50 # The number of fields sampled
n.picks = 10 # The number of picks in the field
# Collect data
y <- rbinom(n.fields, size = n.picks, prob = p.orchid) 
hist(y, xlab = "Count of orchids",
...
So 10 picks in 50 fields tells us little.
...
100 picks in 50 fields reduces the variability in the proportion of observed orchids.
We expect around 40 orchids per field, with much less variability than before.

## Estimator and Estimand

We have been estimating $p$ — let's name what we are doing more carefully.

- Estimand: $p$, the parameter we want to estimate, the true population level parameter
- Estimate: $\hat{p}$, the value computed from the observed data $y_i$
- Estimator: $\hat{p}_{orchid} = \frac{\sum y_i}{n_{picks}\cdot n_{fields}}$, how we are estimating it
...
\begin{equation}
\log f(y_i;n_{picks},p) = y_i\log\{p\}+(n_{picks}-y_i)\log\{1-p\} +constant
\end{equation}
...
n.picks = 100
n.fields <- 50
y <- rbinom(n.fields, size = n.picks, prob = p.orchid) # Collecting data
```
...
- simulation: try many values (inefficient)
- analytically: do mathematics (as in LMs, not always possible)
...
Our function:
\begin{equation}
\log\{\mathcal{L}(\textbf{y};n_{picks})\} = \pause \sum \limits^{n_{fields}}_{i=1} y_i\log\{p\}+(n_{picks}-y_i)\log\{1-p\} +constant
\end{equation}
...
\begin{equation}
\frac{\partial\log\{\mathcal{L}(\textbf{y};n_{picks})\}}{\partial p} \pause = \sum \limits^{n_{fields}}_{i=1} \frac{y_i}{p} - \frac{n_{picks}-y_i}{1-p}
\end{equation}
...
\only<1>{
\begin{equation}
0 = \sum \limits^{n_{fields}}_{i=1} \frac{y_i}{p} - \frac{n_{picks}-y_i}{1-p}
\end{equation}
}

\only<2>{
\begin{equation}
\sum \limits^{n_{fields}}_{i=1} \frac{n_{picks}-y_i}{1-p} = \sum \limits^{n_{fields}}_{i=1} \frac{y_i}{p}
\end{equation}
}
\only<3>{
\begin{equation}
\sum \limits^{n_{fields}}_{i=1} (n_{picks}-y_i)p = \sum \limits^{n_{fields}}_{i=1}  y_i(1-p)
\end{equation}
}
\only<4>{
\begin{equation}
 n_{fields}n_{picks}p - \sum \limits^{n_{fields}}_{i=1}y_ip = \sum \limits^{n_{fields}}_{i=1} y_i- \sum \limits^{n_{fields}}_{i=1}y_ip 
\end{equation}
}
\only<5>{
\begin{equation}
\sum \limits^{n_{fields}}_{i=1} y_i = n_{fields}n_{picks}p
\end{equation}
}
\only<6>{
\begin{equation}
\hat{p} =  \frac{\sum \limits^{n_{fields}}_{i=1}y_i}{n_{fields}n_{picks}}
\end{equation}
}
...
\begin{equation}
\hat{p} =  \frac{\sum \limits^{n_{fields}}_{i=1}y_i}{n_{fields}}
\end{equation}
...
Generally, no ideal properties in finite samples. But as we get more data:
...
\begin{equation}
\frac{\partial^2\log\{\mathcal{L}(\textbf{y};n_{picks})\}}{\partial p^2} = -\sum \limits^{n_{fields}}_{i=1} \frac{y_i}{p^2}+\frac{n_{picks}-y_i}{(1-p)^2}
\end{equation}

<!-- At the MLE, sum(y_i) = n_total * hat{p}, so each term simplifies:
     sum(y_i) / hat{p}^2  =  n_total / hat{p}
     (n_total - sum(y_i)) / (1-hat{p})^2  =  n_total / (1-hat{p})
     Together: n_total * [1/hat{p} + 1/(1-hat{p})] = n_total / [hat{p}*(1-hat{p})]
     => SE = sqrt( hat{p}*(1-hat{p}) / n_total )
     This equals the exact variance of hat{p} from the Binomial distribution. -->
...
<!-- Replacing y_i by its expectation E[y_i] = n_picks*p in the observed Fisher information
     gives the expected Fisher information I(p) = n_fields*n_picks / [p*(1-p)],
     which is where the true p in the CLT variance comes from. -->

With enough data, $\hat{p}$ across repeated samples approaches a Normal distribution:

\begin{equation} 
\hat{p} \sim \mathcal{N}\!\left(p,\; \frac{p(1-p)}{n_{fields}\cdot n_{picks}}\right) \nonumber
\end{equation}

\pause

- Precision grows with more fields \textit{and} more picks per field
- Underpins the $\hat{p} \pm 1.96\cdot\text{SE}$ confidence interval
- Breaks down when $n_{fields} \cdot n_{picks}$ is small or $p$ is near 0 or 1
- Note: this is also shown by \texttt{summary()} in GLMs (more later)
...
n_reps <- 200
for(n in c(5, 50, 500)){
  phat_sim <- rbinom(n_reps, size = n, prob = p.orchid) / n
  hist(phat_sim, main = paste0("n = ", n), xlab = expression(hat(p)),
       freq = FALSE, breaks = 20)
  curve(dnorm(x, p.orchid, sqrt(p.orchid * (1 - p.orchid) / n)),
        add = TRUE, col = "red", lwd = 2)
}
```

Normal approximation in red. Here $n = n_{fields} \cdot n_{picks}$ is the total number of picks.
...
Different methods exist. The $\hat{p} \pm 1.96\,\text{SE}$ interval is not always correct for the Binomial distribution — the **normal approximation** for $\hat{p}$ breaks down when $n$ is small or $p$ is near 0 or 1.
...
is a linear combination of $y_i \sim Binom(1,p)$, from which we know $\biggl(\sum \limits^{n_{fields}}_{i=1} y_i \biggr)\sim Binom(n_{fields}, p)$\newline

\pause

From this we also know $\text{var}(\hat{p}) = \frac{p(1-p)}{n_{fields}}$.
...
abline(v=qbinom(0.025,n.picks*n.fields,p$maximum)/(n.picks*n.fields), col="blue")
abline(v=qbinom(0.975,n.picks*n.fields,p$maximum)/(n.picks*n.fields), col="blue")
```
...
After all, we are looking for a robust recommendation.
