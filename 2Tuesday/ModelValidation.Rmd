...
- $\epsilon_i = y_i - \alpha - \sum \limits^p_{k=1}x_{ik}\beta_k$
- $\hat{\epsilon}_i = y_i - \hat{\alpha} - \sum \limits^p_{k=1}x_{ik}\hat{\beta}_k$
...
$$\hat{\mu}_i = \hat{\alpha} + \sum \limits^p_{k=1}x_{ik}\hat{\beta}_k$$
but we can also write:

$\hat{\boldsymbol{\mu}} = \textbf{H}\textbf{y}$

- $\textbf{H}$ is the "hat" matrix
- $\textbf{H} = \textbf{X}(\textbf{X}^\top\textbf{X})^{-1}\textbf{X}^\top$
...
```{r, echo = FALSE, fig.align = "center", fig.height = 3.5, fig.width = 5, out.width = "80%"}
plot(y3~x3, data = anscombe)
model3 <- lm(y3~x3, data = anscombe)
abline(model3, col = "red")
```
...
```{r, echo = FALSE, fig.align = "center", fig.height = 3.5, fig.width = 4, out.width = "\\linewidth"}
plot(y3~x3, data = anscombe)
model3 <- lm(y3~x3, data = anscombe)
abline(model3, col = "red")
```
