...
\columnsend
\textbf{The basis of many statistical models}

Many results are now asymptotic \tiny (via the normal distribution)\newline
...
$\text{g}^{-1}(\cdot)$ is the **inverse link function** \newline
...
[McCullagh and Nelder (1989)](https://www.utstat.toronto.edu/brunner/oldclass/2201s11/readings/glmbook.pdf) wrote a book that popularised the class.
...
$$\Theta^{t+1} = \Theta^t + {\frac{\partial^2\log\{\mathcal{L}(\textbf{y};\Theta^t)\}}{\partial\Theta\partial\Theta^\top}}^{-1}\frac{\partial\mathcal{L}(\textbf{y};\Theta^t)}{\partial\Theta}$$

- (Newton-Raphson) Can get quite expensive to evaluate.
- Nelder and Wedderburn (1972) instead suggested an algorithm that fits a LM repeatedly.

## Iteratively reweighted least squares (IRLS)

1) Start at some (decent) guess (e.g., $\hat{\symbf{\eta}} = \textbf{y} + \symbf{\epsilon}$) \tiny (Wood 2017) \normalsize\pause
2) Set $\textbf{z}^t = \symbf{\eta} + \frac{\partial \hat{\symbf{\eta}}}{\partial g^{-1}(\hat{\symbf{\eta}})}\{\textbf{y} - g^{-1}(\hat{\symbf{\eta}})\}$\pause
3) Calculate weights $\textbf{w} = 1/\{\frac{\partial \hat{\symbf{\eta}}}{\partial g^{-1}(\hat{\symbf{\eta}})}^2\text{var}(y_i)\}$\pause
4) Fit weighted LM with $\symbf{z}$ as "data" and $\textbf{w}$ as weights\pause
5) Repeat until convergence\pause
...
```{r, echo = FALSE}
hist(MLBattend$wins, xlab = "Wins", main = NULL, cex.lab = 1.5, cex.axis = 1.5)
```
...
OK, let's say we have a "final" model. Now, the real work begins! \newline
...
- Standard errors cannot be back-transformed!
  - Interpretation happens on the __link__ scale
...
binomial: $2\sum \limits^n_{i=1} y_i\log(y_i/\hat{\mu}_i)+(N-y_i)\log\{(N-y_i)/(N-\hat{\mu}_i)\}$
