...
- we also have $p$ covariate effects $\beta_k$
...
where $\textbf{Y}$, $\symbf{\beta}$ and $\symbf{\epsilon}$ are now all vectors of length $n$, where there are $n$ data points. $\textbf{X}$ is an $n \times p$ matrix.
...
- the computer actually doesn't use equation \eqref{mle}
...
![<a href="https://www.vecteezy.com/free-vector/coronavirus">by Vecteezy</a>](temperature.jpg)
...
library(tidyr)
knitr::kable(head(normtemp, 7), format="latex", booktabs = T)%>%kableExtra::kable_styling(position="center")

normtemp$celsius <- (normtemp$temperature- 32) * 5/9;normtemp$gender<-factor(normtemp$gender, labels=c("Male","Female"))
```
...
$y_i = \alpha + female*\beta + left.over.stuff_i$
\newline
and \newline
$y_i = \alpha + heart.rate*\beta + left.over.stuff_i$
...
```
```{r echo = FALSE}
printCoefmat(rbind(gender=c(coef(model1)[1], hr=NA, coef(model1)[2]),
                   hr = c(coef(model2), NA),
                   both = coef(model3)), digits=3)
```
...
- $\beta_1$ is the slope for $x_1$
- $\beta_2$ is the slope of $x_2$
- $\beta_3$ is their joint parameter

$\beta_3$ represents how the effect of $x_1$ or $x_2$ changes with the other covariate. E.g., body size and heart rate on body temperature
...
\vdots \\
  ...
  \end{array}  \right)
$$
A separate regression line for each category. Here, $\alpha$ and $\beta_2$ are the intercept and slope for the regression line of the first category.\newline
$\alpha+\beta_1$ is the intercept and $\beta_2+\beta_3$ is the slope of the regression line for the second category.
...
- Centering: $y_i = (x_i-\bar{\textbf{x}})\beta+ \epsilon_i$
...
See [GAM workshop](https://www.physalia-courses.org/courses-workshops/gams-in-r/)
...
In \texttt{R} we can calculate $\hat{y}_i = \hat{\alpha} + x_i\hat{\beta_1}$ with the `predict` function:
